...
var server = http.createServer(function(request,response){
  ...
  //섞여있는 url을 분석(parsing)하기 위해서는 전담 모듈을 이용하자 url내장모듈
  var parseObj = url.parse(request.url,true); //분석시작
  //분석시,true를 매개변수로 전달하면, 파라미터들을 {json}으로 묶어준다
  //장점? json은 객체표기법이므로, 데이터들을 . 점찍고 마치 객체 다루듯 접근할 수 있기때문에
  //직관성이 높아진다. 즉, 다루기 쉬워진다 (배열보다 낫다^-^)
  console.log("url분석 결과 : ", parseObj);

  response.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});

  if(parseObj.pathname=="/member/registForm"){ //회원가입을 양식디자인 폼을 원하면
